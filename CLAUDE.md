...
- Bundle ID / package — **diverges per platform**: Android `applicationId` = `kz.zhannaslyamova.lms` (published in Google Play under this name); iOS `PRODUCT_BUNDLE_IDENTIFIER` = `kz.zhannaslyamova.lms.ios`. The original `kz.zhannaslyamova.lms` was permanently held by an old Xcode Personal Team registration, Apple Support refused to release it (Individual program can't formally add the developer), so the iOS bundle was suffixed `.ios` instead of churning the Android store listing. Both are locked once published in their respective stores — neither can change.
